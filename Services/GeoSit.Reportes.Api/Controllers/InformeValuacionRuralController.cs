        ...
        public IHttpActionResult Get(long idUnidadTributaria, long? idTramite, string usuario)
        {
            using (var client = new HttpClient() { BaseAddress = new Uri(ConfigurationManager.AppSettings["webApiUrl"]) })
            {
                try
                {
                    ...
                    //DDJJs
                    result = client.GetAsync("api/DeclaracionJurada/GetDeclaracionJuradaVigenteSoR?idUnidadTributaria=" + unidadTributaria.UnidadTributariaId).Result;
                    result.EnsureSuccessStatusCode();
                    ...
                    byte[] bytes = ReporteHelper.GenerarReporte(new Reportes.InformeValuacionRural(), unidadTributaria, usuario, tramite.Numero);
