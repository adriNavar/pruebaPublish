...
{
    public class InformeDDJJUController : ApiController
    {
        public IHttpActionResult Get(long idDeclaracionJurada, long? idTramite, string usuario)
        {
            using (var client = new HttpClient() { BaseAddress = new Uri(ConfigurationManager.AppSettings["webApiUrl"]) })
            {
                try
                {
                    ...
                    byte[] bytes = ReporteHelper.GenerarReporte(new InformeDDJJU(), unidadTributaria, usuario, tramite.Numero);
                    return Ok(Convert.ToBase64String(bytes));
                }
                catch (Exception ex)
                {
                    WebApiApplication.GetLogger().LogError("InformeDDJJU", ex);
                    return NotFound();
                }
            }
        }
